...
import type { Workspace, SessionInfo } from "../../types";
...
type RefreshInterval = "off" | "10s" | "1m";
...
function sessionToRows(session: SessionInfo | undefined, workspaceId: string): [string, string][] {
...
export default function Sidebar({
  ...
  const fetchData = useCallback((isAutoRefresh: boolean) => {
    GetWorkspaces()
      .then((result) => {
        const ws = (result || []) as unknown as Workspace[];
        setWorkspaces(ws);

        // If selected workspace was deleted, deselect
        if (selectedWorkspace && !ws.some((w) => w.id === selectedWorkspace)) {
          onSelectWorkspace(null);
        }

        // Blink on auto refresh
        if (isAutoRefresh) {
          onRefresh();
          setBlinking(true);
  ...
  }, [selectedWorkspace, onSelectWorkspace, onRefresh]);

  useEffect(() => {
    GetWorkspaces()
      .then((result) => setWorkspaces((result || []) as unknown as Workspace[]))
  ...
  useEffect(() => {
    ...
    const ms = refreshInterval === "10s" ? 10_000 : 60_000;
    timerRef.current = setInterval(() => fetchData(true), ms);
    return () => {
    ...
    <>
            ...
            <div className="flex items-center gap-1.5">
              <span
                className={clsx(
                  ...
                  blinking ? "text-blue-500 font-semibold" : "text-zinc-400"
                )}
              >
                Refresh
              </span>
              <select
                ...
                className={clsx(
                  "text-xs px-1.5 py-0.5 rounded border focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors duration-300",
                  blinking
                    ? "border-blue-400 text-blue-500 bg-blue-50 dark:bg-blue-900/30"
                    : "border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-zinc-700 dark:text-zinc-200"
                )}
              >
      ...
      {detailWs && (
        <DetailPopup
          ...
          rows={sessionToRows(detailWs.session, detailWs.id)}
